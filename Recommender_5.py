...
s=distDF.values
k=5                                                                                                          # The number of neighbours to find
...
for i in range(len(wide_artist_array)):
    ...
    unr3=user_mean_rating[nbs3]
    unr4=user_mean_rating[nbs4]
    unr5=user_mean_rating[nbs5]

    dist1=distance_matrix[i][nbs1]
    dist2=distance_matrix[i][nbs2]
    dist3=distance_matrix[i][nbs3]
    dist4=distance_matrix[i][nbs4]
    dist5=distance_matrix[i][nbs5]

    for j in range(len(wide_artist_array[i])):
        
        dummy[i][j] = umr + (   (dist1 * ( wide_artist_array[nbs1][j] - unr1)   + dist2 * (wide_artist_array[nbs2][j] - unr2) + dist3 * (wide_artist_array[nbs3][j] - unr3)+dist4 * (wide_artist_array[nbs4][j] - unr4)+dist5 * (wide_artist_array[nbs5][j] - unr5)) / ( dist1 + dist2 + dist3 + dist4+ dist5)   )   
...
with open('neighbours-5-lastFM.json', 'w') as file:
